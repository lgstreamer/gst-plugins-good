...
/* maximum allowed non-mdat top-level box size for ATSC3.0 */
#define QTDEMUX_ATSC3_MAX_ATOM_SIZE (5*1024)
...
#define QTDEMUX_TREE_NODE_FOURCC(n) ((n) ? \
    QT_FOURCC(((guint8 *) (n)->data) + 4) : FOURCC_null)
...
typedef struct _QtDemuxSampleToGroupEntry QtDemuxSampleToGroupEntry;
...
#define QTDEMUX_IS_CUSTOM_MODE(qtdemux) ((qtdemux)->dash_mode || (qtdemux)->atsc3_mode)
#endif

#define QTDEMUX_EXPOSE_GET_LOCK(demux) (&((demux)->expose_lock))
#define QTDEMUX_EXPOSE_LOCK(demux) G_STMT_START { \
    GST_DEBUG_OBJECT (demux, "Locking from thread %p", g_thread_self()); \
    g_mutex_lock (QTDEMUX_EXPOSE_GET_LOCK (demux)); \
    GST_DEBUG_OBJECT (demux, "Locked from thread %p", g_thread_self()); \
} G_STMT_END

#define QTDEMUX_EXPOSE_UNLOCK(demux) G_STMT_START { \
    GST_DEBUG_OBJECT (demux, "Unlocking from thread %p", g_thread_self()); \
...
struct _QtDemuxSampleToGroupEntry
{
...
enum QtDemuxState
{
  ...
  QTDEMUX_STATE_LOST_SYNC       /* Lost sync in push mode, drop data until ftyp box */
};
...
static gboolean qtdemux_is_keyframe_trick_condition (GstQTDemux * qtdemux);
...
static gboolean qtdemux_sample_description_copy_into_stream (GstQTDemux *
    qtdemux, QtDemuxStreamStsdEntry * stsd_entry, QtDemuxStream * stream);

static gboolean qtdemux_stream_copy_into_sample_description (GstQTDemux *
    qtdemux, QtDemuxStream * stream, gint stsd_index);
...
#if defined (MPEGDASH_MODE) || defined (ATSC3_MODE)
  qtdemux_signals[START_TIME] =
      g_signal_new ("start-time", G_TYPE_FROM_CLASS (klass),
      G_SIGNAL_RUN_LAST, G_STRUCT_OFFSET (GstQTDemuxClass, start_time),
...
get_property_cb (GstPad * pad, GstPad * peer, gpointer usr_data)
{
  ...
  res =
      gst_element_get_smart_properties (GST_ELEMENT_CAST (qtdemux),
      "thumbnail-mode", &qtdemux->thumbnail_mode,
#ifdef MPEGDASH_MODE
      "dash-mode", &qtdemux->dash_mode, "use-svp", &qtdemux->use_svp,
#endif
#ifdef ATSC3_MODE
      "atsc3-mode", &qtdemux->atsc3_mode,
#endif
#ifdef DOLBYHDR_SUPPORT
  ...
  GST_INFO_OBJECT (qtdemux, "thumbnail-mode = %d", qtdemux->thumbnail_mode);
#ifdef MPEGDASH_MODE
  GST_INFO_OBJECT (qtdemux, "dash-mode = %d", qtdemux->dash_mode);
  GST_INFO_OBJECT (qtdemux, "use-svp = %d", qtdemux->use_svp);
#endif
#ifdef ATSC3_MODE
  GST_INFO_OBJECT (qtdemux, "atsc3-mode = %d", qtdemux->atsc3_mode);
#endif
#ifdef DOLBYHDR_SUPPORT
...
gst_qtdemux_init (GstQTDemux * qtdemux)
{
  ...
  qtdemux->collection = NULL;
  qtdemux->state = QTDEMUX_STATE_INITIAL;
  qtdemux->pullbased = FALSE;
  qtdemux->posted_redirect = FALSE;
  qtdemux->upstream_basetime = GST_CLOCK_STIME_NONE;
  qtdemux->new_collection = FALSE;
  ...
  qtdemux->seek_to_key_frame = FALSE;
  gst_segment_init (&qtdemux->segment, GST_FORMAT_TIME);
  qtdemux->flowcombiner = gst_flow_combiner_new ();
  qtdemux->mpu_seq_num = G_MAXUINT32;
  qtdemux->asset_id = NULL;
  qtdemux->has_mmth = FALSE;
  qtdemux->is_mmth_timed = TRUE;
  qtdemux->ignore_hintsample = FALSE;
  g_mutex_init (&qtdemux->expose_lock);

#if defined (MPEGDASH_MODE) || defined (ATSC3_MODE)
  qtdemux->dash_mode = FALSE;
  qtdemux->dash_pts_offset = GST_CLOCK_STIME_NONE;
  qtdemux->dash_fragment_start = -1;
  qtdemux->dash_segment_start = GST_CLOCK_TIME_NONE;
  qtdemux->dash_period_start = 0;
  qtdemux->dash_subtitle_offset = 0;
  qtdemux->dash_subtitle_index = 0;
#endif
#ifdef ATSC3_MODE
  qtdemux->atsc3_mode = FALSE;
  qtdemux->prev_decode_time = GST_CLOCK_TIME_NONE;
  ...
  qtdemux->prev_segment_position = 0;
  ...
  qtdemux->highest_temporal_id = -1;
  qtdemux->preselection_id = -1;
    ...
    case GST_QUERY_CUSTOM:{
      ...
      if (gst_structure_has_name (s, "custom-trickable")) {
        if (!qtdemux->pullbased && qtdemux->isBigData) {
          trickable = FALSE;
        } else if (qtdemux->has_dolby_bl_cand && qtdemux->has_dolby_el_cand
            && qtdemux->fragmented) {
          trickable = FALSE;
        }
...
#if 0                           //CID 89385 (#1 of 1): Structurally dead code (UNREACHABLE)
      gst_query_set_formats (query, 2, GST_FORMAT_TIME, GST_FORMAT_BYTES);
      res = TRUE;
      break;
#endif
    ...
    gboolean use_sparse, gint64 * key_time, gint64 * key_offset)
{
  ...
  for (n = 0; n < qtdemux->n_streams; n++) {
    ...
    if ((qtdemux->demux_rate > 2.0 || qtdemux->demux_rate == 0.5)
        && strstr (GST_PAD_NAME (str->pad), "audio"))
      continue;
...
gst_qtdemux_do_push_seek (GstQTDemux * qtdemux, GstPad * pad, GstEvent * event)
{
  ...
  if (qtdemux->dlna_opval == 0x00 && rate == 0.5) {
    qtdemux->segment.rate = rate;
    qtdemux->segment.start = qtdemux->segment.time = qtdemux->segment.position;
  ...
  qtdemux->rate_changed = TRUE;
  qtdemux->segment.rate = rate;
  /* Handles trick condition */
  if (rate == 0.5 || qtdemux_is_keyframe_trick_condition (qtdemux)) {
    ...
    qtdemux->next_seek_offset = qtdemux->prev_seek_offset = qtdemux->offset;
  }
#endif
  ...
  qtdemux->trick_offset = (qtdemux->dlna_opval == 0x10) ? key_cur : byte_cur;
  ...
  format = (qtdemux->dlna_opval == 0x10) ? GST_FORMAT_TIME : GST_FORMAT_BYTES;
  ...
  event =
      gst_event_new_seek (rate, format, flags, cur_type, qtdemux->trick_offset,
      stop_type, stop);
...
#ifdef MP4_PUSHMODE_TRICK
/* This function based on Byte or Time Format creates seek event
for respective start and stop values */
static gboolean
gst_qtdemux_seek_push (GstQTDemux * demux, guint64 start, guint64 stop)
{
  ...
  GstFormat format;
  format = (demux->dlna_opval == 0x10) ? GST_FORMAT_TIME : GST_FORMAT_BYTES;
  seek_event =
      gst_event_new_seek (1.0, format,
      GST_SEEK_FLAG_SKIP | GST_SEEK_FLAG_FLUSH, GST_SEEK_TYPE_SET, start,
      GST_SEEK_TYPE_NONE, stop);
  ret = gst_pad_push_event (demux->sinkpad, seek_event);
  return ret;
}

/* This function is called from chain function of qtdemux to generate seek events
based on conditions of trick play */
static gboolean
gst_qtdemux_handle_trick (GstQTDemux * demux)
{
  ...
  guint64 start, stop;
  gboolean ret = TRUE;
  demux->rate_changed = FALSE;
  if (demux->segment.rate < 0.0) {
    demux->prev_seek_offset = demux->trick_offset;
    /*Handling cases where prev_seek_offset & rewind_offset becomes
       equal before reaching start of file */
    if (demux->prev_segment_position == demux->segment.position)
      demux->segment.position = demux->segment.position - TIME_ADJUST;

    if (demux->prev_seek_offset != G_MAXUINT64 &&
        demux->segment.position > (guint64) TIME_ADJUST) {
      /*Time adjust using next seek offset in Trick forward time */
      if (demux->dlna_opval == 0x10) {
        demux->prev_seek_offset = demux->next_seek_offset;
        gst_qtdemux_adjust_seek (demux, demux->segment.position - TIME_ADJUST,
            FALSE, &time_offset, &offset);
        demux->next_seek_offset = offset;
      } else
        gst_qtdemux_adjust_seek (demux, demux->segment.position - TIME_ADJUST,
            FALSE, &time_offset, &offset);
      demux->trick_offset = offset;
    } else
      demux->trick_offset = G_MAXUINT64;
    demux->prev_segment_position = demux->segment.position;
    if (demux->trick_offset > 0 && demux->trick_offset != G_MAXUINT64) {
      start = (demux->dlna_opval == 0x10) ? time_offset : offset;
      stop = -1;
      ret = gst_qtdemux_seek_push (demux, start, stop);
    } else {
      gst_qtdemux_push_event (demux, gst_event_new_eos ());
      return TRUE;
    }
  } else {
    demux->next_seek_offset = demux->trick_offset;
    if (demux->next_seek_offset != G_MAXUINT64) {
      /*Time adjust using next seek offset in Trick forward time */
      if (demux->dlna_opval == 0x10) {
        demux->next_seek_offset = demux->prev_seek_offset;
        gst_qtdemux_adjust_seek (demux, demux->segment.position + TIME_ADJUST,
            FALSE, &time_offset, &offset);
        demux->prev_seek_offset = offset;
      } else
        gst_qtdemux_adjust_seek (demux, demux->segment.position + TIME_ADJUST,
            FALSE, &time_offset, &offset);
    } else
      demux->trick_offset = G_MAXUINT64;
    demux->trick_offset = (demux->dlna_opval == 0x10) ? time_offset : offset;
    if (demux->trick_offset != demux->next_seek_offset
        && demux->trick_offset != G_MAXUINT64) {
      start = demux->trick_offset;
      stop = -1;
      ret = gst_qtdemux_seek_push (demux, start, stop);
    }
  }
...
gst_qtdemux_setcaps (GstQTDemux * demux, GstCaps * caps)
{
  ...
  gint highest_temporal_id = -1, preselection_id = -1;

  GST_DEBUG_OBJECT (demux, "Sink set caps: %" GST_PTR_FORMAT, caps);

  structure = gst_caps_get_structure (caps, 0);
  variant = gst_structure_get_string (structure, "variant");
  gst_structure_get_int (structure, "highest-temporal-id",
      &highest_temporal_id);
  gst_structure_get_int (structure, "preselection-id", &preselection_id);

  if (highest_temporal_id != -1) {
    GST_DEBUG_OBJECT (demux, "Seeing highest-temporal-id: %d",
        highest_temporal_id);
    demux->highest_temporal_id = highest_temporal_id;
  }

  if (preselection_id != -1) {
    GST_DEBUG_OBJECT (demux, "Detect preselection-id: %d", preselection_id);
    demux->preselection_id = preselection_id;
  }
...
gst_qtdemux_reset (GstQTDemux * qtdemux, gboolean hard)
{
  ...
  qtdemux->pushed_Audio = FALSE;
  qtdemux->all_audio_pushed = FALSE;
  qtdemux->prev_seek_offset = 0;
#endif
  ...
  if (hard) {
    ...
    if (qtdemux->collection) {
      gst_object_unref (qtdemux->collection);
      qtdemux->collection = NULL;
    }
    qtdemux->n_streams = 0;
    ...
    qtdemux->upstream_basetime = GST_CLOCK_STIME_NONE;
    qtdemux->upstream_basetime_offset = GST_CLOCK_TIME_NONE;
    qtdemux->mpu_seq_num = G_MAXUINT32;
    g_free (qtdemux->asset_id);
    qtdemux->has_mmth = FALSE;
    qtdemux->is_mmth_timed = TRUE;
    qtdemux->ignore_hintsample = FALSE;
    qtdemux->configure_dvr = FALSE;
  ...
  switch (GST_EVENT_TYPE (event)) {
    case GST_EVENT_SEGMENT:
    {
...
#ifdef MP4_PUSHMODE_TRICK
      demux->segment_event_recvd = TRUE;
#endif
      ...
      if (segment.format == GST_FORMAT_BYTES) {
        ...
        if (demux->dlna_opval == 0x10) {
          GST_DEBUG_OBJECT (demux, "Server in Time seek mode");
          offset = segment.base;
        }
      } else {
      ...
      if (demux->rate_changed && demux->n_streams) {
        if (demux->fragmented) {
    ...
    case GST_EVENT_FLUSH_START:
    {
      ...
      demux->segment_event_recvd = FALSE;

      if ((demux->segment.rate < 0.0 || demux->segment.rate > 2.0
              || demux->segment.rate == 0.5) && !demux->rate_changed
          && !demux->pullbased) {
        res = TRUE;
        goto drop;
      } else {
    ...
    {
#ifdef ATSC3_MODE
      demux->prev_decode_time = GST_CLOCK_TIME_NONE;
#endif
    ...
    case GST_EVENT_STREAM_START:
    {
      ...
      if (!demux->collection) {
        demux->collection = gst_stream_collection_new (upstream_id);
      } else if (!demux->pullbased && g_strcmp0 (upstream_id,
              gst_stream_collection_get_upstream_id (demux->collection))) {
        /* drain all data if stream-id changed */
        gint i;
        gboolean has_valid_stream = FALSE;
        GST_DEBUG_OBJECT (demux, "stream-id changed");
        for (i = 0; i < demux->n_streams; i++) {
          if (demux->streams[i]->pad != NULL) {
            has_valid_stream = TRUE;
            break;
        ...
        if (has_valid_stream) {
          /* if we are in here, there was valid stream, so drain all buffers */
          GST_DEBUG_OBJECT (demux, "drain buffers, if available");
          gst_qtdemux_process_adapter (demux, TRUE);
        }
        ...
        gst_object_unref (demux->collection);
        demux->collection = gst_stream_collection_new (upstream_id);

        /* FIXME: in case dolby-vision dual track, stream-collection may cause
         * problem. To prevent regression, let's block this.
         * Please let me fix when stream-collection for dolby-vision dual track is verified */
        if (!demux->is_dolby_hdr)
          demux->new_collection = TRUE;
    ...
    case GST_EVENT_CUSTOM_DOWNSTREAM:
    {
      ...
      if (structure) {
        if (gst_structure_has_name (structure, "dash-period")) {
          gint64 dash_pts_offset;
          guint64 dash_period_start;
          gint64 dash_subtitle_offset;
          guint dash_subtitle_index;

          if (gst_structure_get_int64 (structure, "pts-offset",
                  &dash_pts_offset)) {
            GST_INFO_OBJECT (demux,
                "received dash pts offset : %" GST_STIME_FORMAT,
                GST_STIME_ARGS (dash_pts_offset));
            demux->dash_pts_offset = dash_pts_offset;
          } else {
            GST_INFO_OBJECT (demux, "fail to parse dash pts-offset event");
          }

          if (gst_structure_get_uint64 (structure, "start", &dash_period_start)) {
            GST_INFO_OBJECT (demux,
                "received dash period start : %" G_GUINT64_FORMAT,
                dash_period_start);
            demux->dash_period_start = dash_period_start;
          } else {
            GST_INFO_OBJECT (demux, "fail to parse dash start event");
          }

          if (gst_structure_get_int64 (structure, "subtitle-offset",
                  &dash_subtitle_offset)) {
            GST_INFO_OBJECT (demux,
                "received dash subtitle offset : %" G_GINT64_FORMAT,
                dash_subtitle_offset);
            demux->dash_subtitle_offset = dash_subtitle_offset;
          } else {
            GST_INFO_OBJECT (demux, "fail to parse dash subtitle-offset event");
          }

          if (gst_structure_get_uint (structure, "subtitle-index",
                  &dash_subtitle_index)) {
            GST_INFO_OBJECT (demux, "received dash subtitle index : %d",
                dash_subtitle_index);
            demux->dash_subtitle_index = dash_subtitle_index;
          } else {
            GST_INFO_OBJECT (demux, "fail to parse dash subtitle-index event");
          }
          gst_event_unref (event);
          res = TRUE;
          goto drop;
        } else if (gst_structure_has_name (structure, "utc-offset")) {
          gint64 upstream_basetime;

          if (gst_structure_get_int64 (structure, "offset", &upstream_basetime)) {
            GST_DEBUG_OBJECT (demux, "received utc-offset: %" GST_STIME_FORMAT
                " (%" G_GINT64_FORMAT ")", GST_STIME_ARGS (upstream_basetime),
                upstream_basetime);

            demux->upstream_basetime = upstream_basetime;
          } else {
            GST_ERROR_OBJECT (demux, "fail to get utc-offset");
          }
...
static void
gst_qtdemux_stream_flush_sample_descriptions (GstQTDemux * qtdemux,
    QtDemuxStream * stream)
{
  gint i;

  if (stream->stsd_entries == NULL || stream->stsd_entries_length < 1)
    return;

  for (i = 0; i < stream->stsd_entries_length; i++) {
    QtDemuxStreamStsdEntry *stsd_entry = &stream->stsd_entries[i];
    if (stsd_entry->rgb8_palette) {
      gst_memory_unref (stsd_entry->rgb8_palette);
      stsd_entry->rgb8_palette = NULL;
    }
    if (stsd_entry->caps) {
      gst_caps_unref (stsd_entry->caps);
      stsd_entry->caps = NULL;
    }
  }
...
gst_qtdemux_stream_clear (GstQTDemux * qtdemux, QtDemuxStream * stream)
{
  ...
  gst_qtdemux_stream_flush_sample_descriptions (qtdemux, stream);
}
...
qtdemux_parse_mmpu (GstQTDemux * qtdemux, const guint8 * buffer, gint length)
{
  ...
  /* reset hint track related variables */
  qtdemux->has_mmth = FALSE;
  qtdemux->is_mmth_timed = TRUE;
  qtdemux->ignore_hintsample = FALSE;

  /* only consider at least a sufficiently complete mmpu atom */
  if (length >= 25) {
    guint8 is_complete;
    guint32 mpu_seq_num;
    guint32 asset_id_scheme;
    guint32 asset_id_length;
    gchar *asset_id_value;
    const guint8 *asset_id_ptr;

    is_complete = QT_UINT8 (buffer + 12);
    mpu_seq_num = QT_UINT32 (buffer + 12 + 1);

    is_complete = is_complete >> 7;

    GST_DEBUG_OBJECT (qtdemux, "is complete: %d", is_complete);

    /* FIXME: do we need to do something with mpu_sequence_number? */
    GST_DEBUG_OBJECT (qtdemux, "mpu sequence number: %" G_GUINT32_FORMAT,
        mpu_seq_num);

    asset_id_scheme = QT_FOURCC (buffer + 17);
    asset_id_length = QT_UINT32 (buffer + 17 + 4);

    GST_DEBUG_OBJECT (qtdemux, "asset id scheme: %" GST_FOURCC_FORMAT,
        GST_FOURCC_ARGS (asset_id_scheme));
    GST_DEBUG_OBJECT (qtdemux, "asset id length: %" G_GUINT32_FORMAT,
        asset_id_length);
    if (asset_id_length != length - 25) {
      GST_ERROR_OBJECT (qtdemux,
          "asset id len %" G_GUINT32_FORMAT
          " is different to remaining bytes %d", asset_id_length, length - 25);
      return;
    }

    asset_id_ptr = buffer + 25;
    asset_id_value = g_strndup ((gchar *) asset_id_ptr, asset_id_length);
    GST_DEBUG_OBJECT (qtdemux, "asset id value: %s", asset_id_value);

    if (g_strcmp0 (asset_id_value, qtdemux->asset_id)) {
      GST_DEBUG_OBJECT (qtdemux, "asset id changed from %s to %s",
          GST_STR_NULL (qtdemux->asset_id), GST_STR_NULL (asset_id_value));
      g_free (qtdemux->asset_id);
      qtdemux->asset_id = g_strdup (asset_id_value);
    }

    if (mpu_seq_num != qtdemux->mpu_seq_num) {
      GST_DEBUG_OBJECT (qtdemux,
          "reset upstream_basetime_offset, mpu sequence number changed from %"
          G_GUINT32_FORMAT " to %" G_GUINT32_FORMAT, qtdemux->mpu_seq_num,
          mpu_seq_num);
      qtdemux->mpu_seq_num = mpu_seq_num;
      qtdemux->upstream_basetime_offset = GST_CLOCK_TIME_NONE;
    }

    g_free (asset_id_value);
  }
}

static gboolean
qtdemux_parse_mmth_sample (GstQTDemux * qtdemux, const guint8 * data,
    gsize size, guint32 * sample_index)
{
  gboolean ret = FALSE;
  guint32 seq_num;              /* the sequence num of MFU (not sample's sequence) */

  if (qtdemux->major_brand != FOURCC_mpuf || !qtdemux->has_mmth)
    return FALSE;

  GST_DEBUG_OBJECT (qtdemux, "parsing MMT Hint Sample");

  /* FIXME: only the minimum size of timed hint sample is considered */
  if ((qtdemux->is_mmth_timed && size < 34) ||
      (!qtdemux->is_mmth_timed && size < 6)) {
    GST_ERROR_OBJECT (qtdemux, "hint sample size is too small");
    ret = FALSE;
    goto done;
  }

  seq_num = QT_UINT32 (data);
  GST_LOG_OBJECT (qtdemux, "    seq_num : %" G_GUINT32_FORMAT, seq_num);

  if (qtdemux->is_mmth_timed) {
    gint8 trackrefindex;        /* MEDIA data's track ID.
                                 * From the spec, this seems not to be track ID of hint track */
    guint32 moof_seq_num;       /* the sequence num of moof which this sample belongs */
    guint32 samplenumber;       /* sample number accumulated from the start of MPU.
                                 * (not in current moof).
                                 * TODO: need confirm whether this sequence is starting
                                 * from MOOV or MOOF */
    guint8 priority;            /* FIXME: how to use this ? */
    guint8 dep_counter;         /* FIXME: this also */
    guint32 offset;             /* offset of MEDIA data in mdat
                                 * TODO: need confirm whether this offset is before
                                 * interleaving or after it. */
    guint32 length;             /* length of MEDIA data */
    guint32 muli_size;
    guint32 muli_fourcc;
    gboolean multilayer_flag;

    trackrefindex = QT_UINT8 (data + 4);
    moof_seq_num = QT_UINT32 (data + 5);
    samplenumber = QT_UINT32 (data + 9);
    priority = QT_UINT8 (data + 13);
    dep_counter = QT_UINT8 (data + 14);
    offset = QT_UINT32 (data + 15);
    length = QT_UINT32 (data + 19);

    /* multiLayerInfo box */
    muli_size = QT_UINT32 (data + 23);
    muli_fourcc = QT_FOURCC (data + 27);
    multilayer_flag = (QT_UINT8 (data + 31) >> 7) & 0x1;

    GST_LOG_OBJECT (qtdemux, "    trackrefindex : %d", trackrefindex);
    GST_LOG_OBJECT (qtdemux, "    moof_seq_num : %"
        G_GUINT32_FORMAT, moof_seq_num);
    GST_LOG_OBJECT (qtdemux, "    samplenumber : %"
        G_GUINT32_FORMAT, samplenumber);
    GST_LOG_OBJECT (qtdemux, "    priority : %u", priority);
    GST_LOG_OBJECT (qtdemux, "    dep_counter : %u", dep_counter);
    GST_LOG_OBJECT (qtdemux, "    offset : %" G_GUINT32_FORMAT, offset);
    GST_LOG_OBJECT (qtdemux, "    length : %" G_GUINT32_FORMAT, length);

    GST_LOG_OBJECT (qtdemux, "    muli_size : %" G_GUINT32_FORMAT, muli_size);
    GST_LOG_OBJECT (qtdemux, "    fourcc : %"
        GST_FOURCC_FORMAT, GST_FOURCC_ARGS (muli_fourcc));
    GST_LOG_OBJECT (qtdemux, "    multilayer_flag : %d", multilayer_flag);


    if (muli_fourcc != FOURCC_muli) {
      GST_ERROR_OBJECT (qtdemux, "seems not to be MMT hint sample");
      ret = FALSE;
      goto done;
    } else if ((multilayer_flag && size != 13) ||
        (!multilayer_flag && muli_size != 11)) {
      GST_ERROR_OBJECT (qtdemux, "wrong multilayer box size");
      ret = FALSE;
      goto done;
    }

    /* samplenumber of the first sample in MPU is 1, but qtdemux will increase
     * it from zero. need to be compensated */
    *sample_index = samplenumber - 1;

    /* samplenumber should be started from 1 */
    if (samplenumber == 0) {
      GST_ERROR_OBJECT (qtdemux, "Invalid samplenumber, ignore");
      qtdemux->ignore_hintsample = TRUE;
    }
  } else {
    guint16 item_id = QT_UINT16 (data + 4);
    GST_TRACE_OBJECT (qtdemux, "    item_ID : %" G_GUINT16_FORMAT, item_id);
  }
  ...
  if (!stream->parsed_trex && qtdemux->moov_node) {
    ...
    if (mvex) {
      trex = qtdemux_tree_get_child_by_type_full (mvex, FOURCC_trex,
          &trex_data);
      while (trex) {
        guint32 id = 0, dur = 0, size = 0, flags = 0, index = 0;
        ...
        if (id != stream->track_id)
          goto next;
        if (!gst_byte_reader_get_uint32_be (&trex_data, &index))
          goto next;
        ...
        GST_DEBUG_OBJECT (qtdemux, "fragment defaults for stream %d; "
            ...
            index, dur, size, flags);

        stream->parsed_trex = TRUE;
        stream->stsd_sample_description_id = index - 1;
...
#define ABS_CLOCK_DIFF(s) (((s) >= 0) ? (s) : (-1 * (s)))
    ...
    gint64 * base_offset, gint64 * running_offset, gint64 decode_ts)
{
...
#ifdef MPEGDASH_MODE
  if (qtdemux->dash_mode) {
    timestamp = 0;

    if (qtdemux->dash_fragment_start != -1) {
      timestamp = qtdemux->dash_fragment_start;
    }

    if (qtdemux->dash_pts_offset != GST_CLOCK_STIME_NONE &&
        qtdemux->dash_segment_start != GST_CLOCK_TIME_NONE) {
      guint64 abs_diff = ABS_CLOCK_DIFF (qtdemux->dash_pts_offset);
      timestamp = QTSTREAMTIME_TO_GSTTIME (stream, timestamp);

      /* Add base time */
      timestamp += qtdemux->dash_segment_start;

      if (qtdemux->dash_pts_offset >= 0) {
        timestamp += abs_diff;
      } else {
        if (timestamp < abs_diff) {
          GST_WARNING_OBJECT (qtdemux, "Cannot compensate DASH pts offset");
          timestamp = 0;
        } else {
          timestamp -= abs_diff;
        }
      }
      timestamp = GSTTIME_TO_QTSTREAMTIME (stream, timestamp);
    }
  ...
  if (qtdemux->fragment_start != -1) {
    ...
    if (stream->n_samples == 0) {
      ...
      if (qtdemux->upstream_basetime != GST_CLOCK_STIME_NONE) {
        guint64 abs_diff = ABS_CLOCK_DIFF (qtdemux->upstream_basetime);
        guint64 compensated_gst_ts = gst_ts;
        if (qtdemux->upstream_basetime >= 0) {
          compensated_gst_ts += abs_diff;
        } else {
          if (compensated_gst_ts < abs_diff) {
            GST_WARNING_OBJECT (qtdemux,
                "Cannot compensate upstream_basetime %" GST_STIME_FORMAT,
                GST_STIME_ARGS (abs_diff));
            compensated_gst_ts = 0;
          } else {
            compensated_gst_ts -= abs_diff;
          }
          GST_DEBUG_OBJECT (qtdemux, "Add upstream_basetime %" GST_STIME_FORMAT
              " to baseDecodeTime", GST_STIME_ARGS (abs_diff));
        }
        timestamp = GSTTIME_TO_QTSTREAMTIME (stream, compensated_gst_ts);
      }
    } else {
      /* subsequent fragments extend stream */
      timestamp =
  ...
  *stream = qtdemux_find_stream (qtdemux, track_id);
  ...
  if (flags & TF_BASE_DATA_OFFSET) {
    if (!gst_byte_reader_get_uint64_be (tfhd, (guint64 *) base_offset))
      goto invalid_track;
    /* Because each MPU is independent, base_data_offset should be compensated
     * with the offset of MPU */
    if (qtdemux->upstream_format_is_time && qtdemux->major_brand == FOURCC_mpuf) {
      *base_offset += qtdemux->mpu_offset;
    }
  }
  ...
  if (flags & TF_SAMPLE_DESCRIPTION_INDEX) {
    guint32 index;
    if (!gst_byte_reader_get_uint32_be (tfhd, &index))
      goto invalid_track;
    ...
    guint64 * decode_time)
{
  ...
  GST_DEBUG_OBJECT (qtdemux, "Track fragment decode time: %" G_GUINT64_FORMAT,
      *decode_time);
    ...
    QtDemuxStream * stream)
{
  ...
  GST_DEBUG_OBJECT (qtdemux, "target_group->sbgp_entries_length: %d",
      target_group->sbgp_entries_length);

  for (i = 0; i < target_group->sbgp_entries_length; i++) {
    QtDemuxSampleToGroupEntry *group_entry = &target_group->sbgp_entries[i];
    GST_DEBUG_OBJECT (qtdemux, "group_entry->sample_count: %d",
        group_entry->sample_count);
...
/* Parses Sample Encryption Box('senc') which contains the sample
 * specific encryption data, including the initialization vectors
 * needed for decryption and, optionally, alternative decryption
 * parameters ; the format of this information is defined in
 * Common File Format(CFF) derived from the ISO Base Media File
 * Format 'iso6' brand specified in [ISO].
...
static gboolean
qtdemux_parse_seig (GstQTDemux * qtdemux, QtDemuxStream * stream,
    QtDemuxSampleToGroupEntry * group_entry, GstByteReader * br)
{
    ...
    GstByteReader * br)
{
  ...
  for (i = 0; i < entry_count; i++) {
    ...
    if (!qtdemux_parse_seig (qtdemux, stream, group_entry, br))
      return FALSE;
    ...
    guint64 moof_offset, QtDemuxStream * stream)
{
  ...
  GstByteReader sbgp_data, sgpd_data;
  ...
  while (traf_node) {
    ...
    sbgp_node =
        ...
        &sbgp_data);
    while (sbgp_node) {
      qtdemux_parse_sbgp (qtdemux, stream, &sbgp_data);
      /* iterate all siblings */
    ...
    while (sgpd_node) {
      qtdemux_parse_sgpd (qtdemux, stream, &sgpd_data);
      /* iterate all siblings */
      sgpd_node = qtdemux_tree_get_sibling_by_type_full (sgpd_node, FOURCC_sgpd,
          &sgpd_data);
    }
    ...
    if (tfdt_node) {
      ...
      if (qtdemux->major_brand == FOURCC_mpuf) {
        if (frag_num == 1 && qtdemux->upstream_format_is_time &&
            GST_CLOCK_TIME_IS_VALID (decode_time_ts) && decode_time_ts != 0) {
          /* FIXME: the first pts should be indicated by mpu_presentation_time.
           * In some erroneous cases, however, it's possible that both mpu_presentation_time
           * (indicated by upstream) and non-zero baseDecodeTime (written in tfdt)
           * exist. To handle this case, let's subtract the first moof's baseDecodeTime
           * to the all moof's which belong to the mpu */
          GST_DEBUG_OBJECT (qtdemux, "nonzero baseDecodeTime %" GST_TIME_FORMAT
              " in the tfdt of the fisrt moof", GST_TIME_ARGS (decode_time_ts));
          qtdemux->upstream_basetime_offset = decode_time_ts;
        }
      } else {
        qtdemux->upstream_basetime_offset = GST_CLOCK_TIME_NONE;
      }
      ...
      if (qtdemux->upstream_format_is_time &&
          GST_CLOCK_STIME_IS_VALID (qtdemux->upstream_basetime)) {
        if (GST_CLOCK_TIME_IS_VALID (qtdemux->upstream_basetime_offset)
            && qtdemux->upstream_basetime >= qtdemux->upstream_basetime_offset) {
          GST_DEBUG_OBJECT (qtdemux, "Add (upstream_basetime %" GST_STIME_FORMAT
              " - upstream_basetime_offset %" GST_TIME_FORMAT
              ") to decode time", GST_STIME_ARGS (qtdemux->upstream_basetime),
              GST_TIME_ARGS (qtdemux->upstream_basetime_offset));
          decode_time_ts +=
              (qtdemux->upstream_basetime - qtdemux->upstream_basetime_offset);
        } else {
          GST_DEBUG_OBJECT (qtdemux, "Add upstream_basetime %" GST_STIME_FORMAT
              " to decode time", GST_STIME_ARGS (qtdemux->upstream_basetime));
          decode_time_ts += qtdemux->upstream_basetime;
        }
      }

      GST_INFO_OBJECT (qtdemux, "decode time %" G_GINT64_FORMAT
...
#ifdef ATSC3_MODE
      /* FIXME: do we need to handle PTS discont. as follows ? */
      if (GST_CLOCK_TIME_IS_VALID (qtdemux->prev_decode_time) &&
          (qtdemux->prev_decode_time > decode_time_ts) && qtdemux->atsc3_mode &&
          (qtdemux->n_video_streams > 0 || qtdemux->n_audio_streams > 0)) {
        GST_INFO_OBJECT (qtdemux, "decode time discont. from %"
            GST_TIME_FORMAT " to %" GST_TIME_FORMAT,
            GST_TIME_ARGS (qtdemux->prev_decode_time),
            GST_TIME_ARGS (decode_time_ts));

        GST_ELEMENT_ERROR (qtdemux, STREAM, DEMUX,
            (_("PTS discont, Cannot be played.")), (NULL));
      }

      qtdemux->prev_decode_time = decode_time_ts;
#endif

#if defined (MPEGDASH_MODE) || defined (ATSC3_MODE)
      qtdemux->dash_fragment_start = decode_time;
      if (QTDEMUX_IS_CUSTOM_MODE (qtdemux) && qtdemux->pending_newsegment) {
        GstSegment segment;
        guint64 updated_start_time = GST_CLOCK_TIME_NONE;
        guint64 time = GST_CLOCK_TIME_NONE;

        gst_event_copy_segment (qtdemux->pending_newsegment, &segment);
        if (segment.format == GST_FORMAT_TIME) {
          segment.start = decode_time_ts;

          g_signal_emit (qtdemux, qtdemux_signals[START_TIME], 0,
              &segment.start, &updated_start_time, &time);

          segment.start = updated_start_time;
          segment.time = time;
          segment.position = time;

          GST_INFO_OBJECT (qtdemux,
              "segment start time %" GST_TIME_FORMAT ", time %" GST_TIME_FORMAT,
              GST_TIME_ARGS (segment.start), GST_TIME_ARGS (segment.time));

          if (qtdemux->dash_segment_start == GST_CLOCK_TIME_NONE) {
            qtdemux->dash_segment_start = segment.start;
          } else {
            segment.start = qtdemux->dash_segment_start;
          }
    ...
    /* In case of MMT, we will reset samples per MPU (not per moof) */
    if (qtdemux->upstream_format_is_time && qtdemux->major_brand != FOURCC_mpuf)
      gst_qtdemux_stream_flush_samples_data (qtdemux, stream);
    ...
    gboolean keyframe, GstClockTime position, guint64 byte_position)
{
  ...
  if (stream->protected
    ...
    if (info->default_properties &&
        gst_structure_get_boolean (info->default_properties, "encrypted",
            &is_encrypted) && !is_encrypted) {
      if (!gst_buffer_add_protection_meta (buf,
              gst_structure_copy (info->default_properties)))
        GST_ERROR_OBJECT (qtdemux, "failed to attach cenc metadata to buffer");
      GST_DEBUG_OBJECT (qtdemux, "pass clear cenc buffer");
      goto push;
    }
...
push:
  if (stream->fourcc == FOURCC_stpp) {
    if (!gst_buffer_add_subtitle_data_meta (buf, qtdemux->dash_period_start,
            qtdemux->dash_subtitle_offset, qtdemux->dash_subtitle_index)) {
      GST_ERROR_OBJECT (qtdemux,
          "failed to attach subtitle metadata to buffer");
    }
  }
...
gst_qtdemux_loop_state_movie (GstQTDemux * qtdemux)
{
  ...
  if (qtdemux_is_keyframe_trick_condition (qtdemux)
      && g_strrstr (GST_PAD_NAME (stream->pad), "video") && !keyframe
      && stream->n_sample_syncs > 1) {
    qtdemux->seek_to_key_frame = TRUE;
    stream->discont = TRUE;
    goto next;
  } else {
    qtdemux->seek_to_key_frame = FALSE;
  ...
  if (g_strrstr (GST_PAD_NAME (stream->pad), "video")
      && stream->fourcc == FOURCC_avc1) {
...
gst_qtdemux_loop (GstPad * pad)
{
  ...
  switch (qtdemux->state) {
    ...
    case QTDEMUX_STATE_MOVIE:
      ...
      if (qtdemux->segment.rate < 0 && (qtdemux->seek_to_key_frame
              || ret == GST_FLOW_EOS)) {
...
next_entry_size (GstQTDemux * demux)
{
  ...
  for (i = 0; i < demux->n_streams; i++) {
    ...
    if (!((stream->subtype == FOURCC_vide) || (stream->subtype == FOURCC_soun))
      ...
      GST_LOG_OBJECT (demux,
          "ignore subtitle sample located before demux offset (stream %d)", i);
      continue;
    }
  ...
  while (((gst_adapter_available (demux->adapter)) >= demux->neededbytes) &&
    ...
    switch (demux->state) {
      ...
      case QTDEMUX_STATE_LOST_SYNC:
      {
        ...
        if (demux->state == QTDEMUX_STATE_LOST_SYNC) {
          if (demux->atsc3_mode) {
            if (fourcc == FOURCC_ftyp || fourcc == FOURCC_moov ||
                fourcc == FOURCC_styp || fourcc == FOURCC_sidx ||
                fourcc == FOURCC_moof || fourcc == FOURCC_mdat) {
              GST_DEBUG_OBJECT (demux,
                  "[ATSC] Find sync, Carrying on normally");
              demux->state = QTDEMUX_STATE_INITIAL;
            } else {
              gst_adapter_clear (demux->adapter);
              demux->neededbytes = 16;
              demux->state = QTDEMUX_STATE_LOST_SYNC;
              break;
            }
          } else {
            if (fourcc == FOURCC_ftyp) {
              GST_DEBUG_OBJECT (demux, "Find sync, Carrying on normally");
              demux->state = QTDEMUX_STATE_INITIAL;
            } else {
              demux->todrop = gst_adapter_available (demux->adapter);
              GST_LOG_OBJECT (demux, "Lost sync, drop %u byte", demux->todrop);
              if (demux->atsc3_mode &&
                  gst_adapter_available (demux->adapter) < demux->todrop) {
                GST_ELEMENT_ERROR (demux, STREAM, DEMUX,
                    (_("LOST SYNC, need to reset demux")),
                    ("LOST SYNC, need to reset demux"));
                return GST_FLOW_ERROR;
              }
        ...
        if (fourcc == FOURCC_mdat) {
          ...
          if (demux->n_streams > 0 && (next_entry != -1 || !demux->fragmented)) {
            ...
            /* re-calculate sample offset for interleaved MMT (AU track + Hint track) */
            if (demux->upstream_format_is_time &&
                demux->major_brand == FOURCC_mpuf && demux->has_mmth) {
              guint i;
              QtDemuxStream *hint_stream = NULL, *media_stream = NULL;
              guint32 hint_sample_idx = -1, media_sample_idx = -1;
              if (demux->n_streams > 2)
                GST_ERROR_OBJECT (demux, "# streams over two");

              for (i = 0; i < demux->n_streams; i++) {
                QtDemuxStream *tmp = demux->streams[i];
                guint32 sample_index = tmp->sample_index;
                if (tmp->subtype == FOURCC_hint) {
                  hint_stream = tmp;
                  hint_sample_idx = sample_index;
                  if (hint_sample_idx == -1)
                    hint_sample_idx = 0;
                } else {
                  media_stream = tmp;
                  media_sample_idx = sample_index;
                  if (media_sample_idx == -1)
                    media_sample_idx = 0;
                }
              }

              if (hint_stream && media_stream) {
                gint64 running_offset = demux->offset + 8;
                for (i = media_sample_idx; i < media_stream->n_samples; i++) {
                  hint_stream->samples[hint_sample_idx].offset = running_offset;
                  running_offset += hint_stream->samples[hint_sample_idx].size;

                  media_stream->samples[media_sample_idx].offset =
                      running_offset;
                  running_offset +=
                      media_stream->samples[media_sample_idx].size;
                ...
                GST_ERROR_OBJECT (demux, "no medai track or hint track");
              }
            }
          } else {
          ...
          if (demux->atsc3_mode) {
            ...
            GST_ELEMENT_ERROR (demux, STREAM, DEMUX,
                ...
                ("atom %" GST_FOURCC_FORMAT " has bogus size %"
                    G_GUINT64_FORMAT, GST_FOURCC_ARGS (fourcc), size));
            ret = GST_FLOW_ERROR;
          }
          break;
        } else {
          ...
          if (demux->mdatbuffer && !(fourcc == FOURCC_moov
            ...
            goto buffer_data;
#ifdef ATSC3_MODE
          } else if (fourcc != FOURCC_moov && fourcc != FOURCC_moof &&
              size > QTDEMUX_ATSC3_MAX_ATOM_SIZE && demux->atsc3_mode &&
              demux->n_streams > 0) {
            GST_ELEMENT_ERROR (demux, STREAM, DEMUX,
                (_("This file is invalid and cannot be played.")),
                ("[ATSC3] atom %" GST_FOURCC_FORMAT " has bogus size %"
                    G_GUINT64_FORMAT, GST_FOURCC_ARGS (fourcc), size));
#endif
          }
          demux->neededbytes = size;
      ...
      case QTDEMUX_STATE_HEADER:{
        ...
        if (fourcc == FOURCC_moov) {
          ...
          if (demux->got_moov && demux->n_streams > 0 &&
            ...
            if (!demux->got_moov || demux->new_collection) {
              QTDEMUX_EXPOSE_LOCK (demux);
...
#if defined (MPEGDASH_MODE) || defined (ATSC3_MODE)
              if (!QTDEMUX_IS_CUSTOM_MODE (demux))
          ...
          if ((demux->got_moov || demux->media_caps) && demux->fragmented) {
            ...
            if (demux->fragment_start_offset == -1
              ...
              if (demux->upstream_format_is_time
                  && demux->major_brand == FOURCC_mpuf) {
                GST_DEBUG_OBJECT (demux, "major brand: %" GST_FOURCC_FORMAT
                    ", update upstream_basetime: %" GST_TIME_FORMAT,
                    GST_FOURCC_ARGS (FOURCC_mpuf), GST_TIME_ARGS (prev_pts));
#ifdef ATSC3_MODE
                /* MMT pts wrap-around case
                 * Please don't be confused by below condition.
                 * "upstream_basetime" is the base PTS of previous MPU buffer
                 * and "prev_pts" is PTS of current MPU buffer
                 */
                if (demux->atsc3_mode &&
                    GST_CLOCK_STIME_IS_VALID (demux->upstream_basetime) &&
                    GST_CLOCK_TIME_IS_VALID (prev_pts) &&
                    demux->upstream_basetime > prev_pts &&
                    (demux->n_video_streams > 0
                        || demux->n_audio_streams > 0)) {
                  GST_INFO_OBJECT (demux,
                      "MMT upstream_basetime wrap-around" "from %"
                      GST_STIME_FORMAT " to %" GST_TIME_FORMAT,
                      GST_STIME_ARGS (demux->upstream_basetime),
            ...
            /* we may get new stream-start event without moov atom.
             * maybe, this is the case that DASH period change without
             * codec data change, and usually, PTS time-line only be changed */
            if (demux->new_collection) {
              gint n;
              for (n = 0; n < demux->n_streams; n++) {
                QtDemuxStream *stream = demux->streams[n];
                gst_qtdemux_configure_stream (demux, stream);
              ...
              gst_element_post_message (GST_ELEMENT_CAST (demux),
                  gst_message_new_stream_collection (GST_OBJECT_CAST (demux),
                      demux->collection));
              /* Send collection event */
              gst_qtdemux_push_event (demux,
                  gst_event_new_stream_collection (demux->collection));

              demux->new_collection = FALSE;
            }
          ...
          if (demux->major_brand == FOURCC_mpuf) {
            demux->mpu_offset = demux->offset;
            GST_DEBUG_OBJECT (demux, "Update mpu offset %" G_GUINT64_FORMAT,
                demux->mpu_offset);
          }
        ...
        } else if (fourcc == FOURCC_mmpu) {
          GST_DEBUG_OBJECT (demux, "Parsing [mmpu]");
          qtdemux_parse_mmpu (demux, data, demux->neededbytes);
        } else {
        ...
        if (demux->mdatbuffer && demux->n_streams) {
          ...
          if (demux->atsc3_mode &&
            ...
            GST_ELEMENT_ERROR (demux, STREAM, DEMUX,
                (_("Cannot flush demux, data is not enouth")),
                ("Cannot flush demux, data is not enouth"));
            return GST_FLOW_ERROR;
          }
      ...
      case QTDEMUX_STATE_BUFFER_MDAT:{
        ...
        if (demux->atsc3_mode && buf == NULL) {
          GST_ELEMENT_ERROR (demux, STREAM, DEMUX,
              (_("Cannot take buffer, data is not enouth")),
              ("Cannot take buffer, data is not enouth"));
          return GST_FLOW_ERROR;
        }
      ...
      case QTDEMUX_STATE_MOVIE:{
        ...
        gboolean keyframe;
        gboolean rewind_check, forward_check;
        GST_DEBUG_OBJECT (demux,
            "BEGIN // in MOVIE for offset %" G_GUINT64_FORMAT, demux->offset);
#ifdef MP4_PUSHMODE_TRICK

        rewind_check = (demux->segment.rate < 0.0 && ((demux->pushed_Iframe
                    && demux->all_audio_pushed)
                || demux->offset >= demux->prev_seek_offset)
            && demux->segment_event_recvd);
        forward_check = (demux->segment.rate > 2.0
            && ((demux->pushed_Iframe && demux->pushed_Audio)
                || demux->offset <= demux->next_seek_offset)
            && demux->segment_event_recvd);
        if (rewind_check || forward_check) {
          demux->all_audio_pushed = FALSE;
          demux->pushed_Audio = FALSE;
          demux->pushed_Iframe = FALSE;

          gst_qtdemux_handle_trick (demux);
          goto eos;
        } else if (demux->segment.rate == 0.5 && (demux->dlna_opval == 0x01 ||
                demux->dlna_opval == 0x11) && demux->pushed_Iframe) {
          demux->pushed_Iframe = FALSE;
          demux->rate_changed = FALSE;
          qtdemux_seek_offset (demux, demux->offset);
        } else {
#endif
          if (demux->fragmented) {
            ...
            if (G_LIKELY (demux->todrop < demux->mdatleft)) {
              ...
              if (demux->restoredata_buffer) {
                if (demux->atsc3_mode &&
                    gst_adapter_available (demux->adapter) != 0) {
                  GST_ELEMENT_ERROR (demux, STREAM, DEMUX,
                      (_("Cannot restore buffer, need to reset demux")),
                      ("Cannot restore buffer, need to reset demux"));
                  return GST_FLOW_ERROR;
                }
          ...
          if (demux->todrop) {
            if (demux->cenc_aux_info_offset > 0) {
              ...
              if (!qtdemux_parse_cenc_aux_info (demux, demux->streams[0], &br,
                      demux->cenc_aux_info_sizes,
                      demux->cenc_aux_sample_count)) {
          ...
          for (i = 0; i < demux->n_streams; i++) {
            ...
            if (stream->sample_index >= stream->n_samples)
              continue;
            GST_LOG_OBJECT (demux,
                "Checking stream %d (sample_index:%d / offset:%"
                G_GUINT64_FORMAT " / size:%d)", i, stream->sample_index,
          ...
          if (!demux->pullbased && !demux->isInterleaved) {
            goto unknown_stream;
          }
          ...
          /*
           * From ISOBMFF spec, each sample which belongs to each other tracks
           * cannot be interleaved in a mdat. e.g., front half of mdat should be for track A,
           * and the other should be for track B based on the ISOBMFF spec.
           * However, MMT spec said that hint sample and media sample should be interleaved
           * from sender side, and receiver must identiy the media sample
           * based on corresponding hint sample.
           * So, we cannot believe sample offset constructed by trun, because the trun
           * indicates the mdat structure of before interleaved.
           *
           *    <case1 : ISOBMFF Spec.>         <case2 : MMT Spec.>
           *    <subcase 1>   <subcase2>
           *    [  mdat  ]    [  mdat  ]            [  mdat  ]
           *    |  AU#1  |    | hint#1 |            | hint#1 |
           *    |  AU#2  |    | hint#2 |  convert   |  AU#1  |
           *    |  ....  | OR |  ....  |  =======>  | hint#2 |
           *    | hint#1 |    |  AU#1  |            |  AU#2  |
           *    | hint#2 |    |  AU#2  |            |  ....  |
           *    |  ....  |    |  ....  |            |  ....  |
          ...
          if (demux->upstream_format_is_time &&
              demux->major_brand == FOURCC_mpuf && demux->has_mmth &&
              demux->is_mmth_timed && stream->subtype == FOURCC_hint) {
            guint32 sample_index = -1;
            const guint8 *data;
            guint64 dropped = 0;

            if (gst_adapter_available (demux->adapter) == 0) {
              GST_ELEMENT_ERROR (demux, STREAM, DEMUX,
                  (_("Data is not available, reset demux")),
                  ("Data is not available, reset demux"));
              return GST_FLOW_ERROR;
            }

            data = gst_adapter_map (demux->adapter,
                gst_adapter_available (demux->adapter));

            if (qtdemux_parse_mmth_sample (demux, data,
                    gst_adapter_available (demux->adapter), &sample_index)) {
              if (demux->ignore_hintsample) {
                GST_DEBUG_OBJECT (demux, "ignore resync by hint sample");
              } else if (sample_index != stream->sample_index) {
                GST_ERROR_OBJECT (demux, "sample_index jump from %"
                    G_GUINT32_FORMAT " to %" G_GUINT32_FORMAT,
                    stream->sample_index, sample_index);
                for (i = 0; i < demux->n_streams; i++) {
                  QtDemuxStream *stream_iter = demux->streams[i];
                  stream_iter->sample_index = sample_index;
                }
                dropped =
                    stream->samples[stream->sample_index].offset -
                    demux->offset;

                GST_ERROR_OBJECT (demux, "%" G_GUINT64_FORMAT
                    " byte MFU dropped", dropped);

                demux->neededbytes = stream->samples[stream->sample_index].size;
                demux->offset = stream->samples[stream->sample_index].offset;
                demux->mdatleft -= dropped;
              }
              gst_adapter_unmap (demux->adapter);
            } else {
              GstBuffer *restore_data;
              gst_adapter_unmap (demux->adapter);

              if (gst_adapter_available (demux->adapter) == 0) {
                GST_ELEMENT_ERROR (demux, STREAM, DEMUX,
                    (_("Hint sample is broken but no available data")),
                    ("Hint sample is broken but no available data"));
                return GST_FLOW_ERROR;
              }

              restore_data = gst_adapter_take_buffer (demux->adapter,
                  gst_adapter_available (demux->adapter));

              GST_ERROR_OBJECT (demux, "fail to parse hint sample. Reset");
              gst_qtdemux_reset (demux, FALSE);

              /* upstream will not resend segment event, so let's do this ourselves */
              demux->upstream_format_is_time = TRUE;
              demux->state = QTDEMUX_STATE_LOST_SYNC;
              gst_adapter_push (demux->adapter, restore_data);
...
#ifdef MP4_PUSHMODE_TRICK
            if (qtdemux_is_keyframe_trick_condition (demux)
                && (!demux->pullbased)) {
              if (strstr (GST_PAD_NAME (stream->pad), "video")) {

                if (demux->pushed_Audio) {
                  demux->all_audio_pushed = TRUE;
                }
              ...
              } else if (strstr (GST_PAD_NAME (stream->pad), "audio")) {
                GstBuffer *outbuf;
                ...
                ret =
                    gst_qtdemux_decorate_and_push_buffer (demux, stream,
                    outbuf, dts, pts, duration, keyframe, dts, demux->offset);
                demux->pushed_Audio = TRUE;
                if (demux->segment.rate > 2.0 && demux->pushed_Iframe)
                  demux->segment_event_recvd = FALSE;
              ...
              if (G_UNLIKELY (demux->segment.stop != -1
                ...
                if (ret == GST_FLOW_EOS) {
                  GST_DEBUG_OBJECT (demux,
                      "All streams are EOS, signal upstream");
                  goto eos;
                }
              } else {
                GstBuffer *outbuf;
                ...
                if (demux->atsc3_mode && outbuf == NULL) {
                  GST_ELEMENT_ERROR (demux, STREAM, DEMUX,
                      (_("Cannot take buffer, data is not enouth")),
                      ("Cannot take buffer, data is not enouth"));
                  return GST_FLOW_ERROR;
                ...
                if (demux->segment.rate == 0.5 && keyframe
                    && strstr (GST_PAD_NAME (stream->pad), "video")) {
                  demux->pushed_Iframe = TRUE;
                }
                ...
                if (demux->is_dolby_hdr && demux->has_dolby_bl_cand &&
          ...
          GST_LOG_OBJECT (demux,
              "increasing offset %" G_GUINT64_FORMAT " by %u", demux->offset,
              demux->neededbytes);
          demux->offset += demux->neededbytes;
    ...
    guint length)
{
  ...
  if (type->flags & QT_FLAG_CONTAINER) {
    ...
    switch (fourcc) {
      ...
      case FOURCC_dvav:
      {
        GST_MEMDUMP_OBJECT (qtdemux, "dvav", buffer, end - buffer);
        qtdemux_parse_container (qtdemux, node, buffer + 0x56, end);
      ...
      case FOURCC_dvhe:
      {
        GST_MEMDUMP_OBJECT (qtdemux, "dvhe", buffer, end - buffer);
        qtdemux_parse_container (qtdemux, node, buffer + 0x56, end);
        qtdemux->has_dolby_el_cand = TRUE;
        break;
      }
      case FOURCC_dvh1:
      {
        GST_MEMDUMP_OBJECT (qtdemux, "dvh1", buffer, end - buffer);
        qtdemux_parse_container (qtdemux, node, buffer + 0x56, end);
        qtdemux->has_dolby_el_cand = TRUE;
        break;
      }
...
gst_qtdemux_configure_stream (GstQTDemux * qtdemux, QtDemuxStream * stream)
{
  ...
  if (stream->subtype == FOURCC_vide) {
      ...
      if (!fps_available && qtdemux->fragmented && stream->pad) {
        ...
        if (prev_caps) {
          ...
          if (s && gst_structure_has_field (s, "framerate")) {
            gst_structure_get_fraction (s, "framerate", &stream->fps_n,
                &stream->fps_d);
            fps_available = TRUE;
          }
    ...
    if (stream->caps) {
      stream->caps = gst_caps_make_writable (stream->caps);
      gst_caps_set_simple (stream->caps,
          "track-num", G_TYPE_INT, (int) qtdemux->n_sub_streams, NULL);
    }
  }
  ...
  if (stream->pad) {
    ...
    if (stream->protected) {
      if (!gst_qtdemux_configure_protected_caps (qtdemux, stream)) {
        /* Notify DRM Rights Error when failed to configure protection system.
         * error-state: 0(No license)
         * content-id: unknown
         * drm-system-id: unknown
         * rights-issuer-url: unknown */
        GstStructure *s = gst_structure_new ("drm-rights-error", "error-state",
            G_TYPE_UINT, 0, "content-id", G_TYPE_STRING, "unknown",
            "drm-system-id", G_TYPE_STRING, "unknown", "rights-issuer-url",
            G_TYPE_STRING, "unknown", NULL);

        GST_DEBUG_OBJECT (qtdemux,
            "Posting drm-rights-error msg to application: %" GST_PTR_FORMAT, s);

        gst_element_post_message (GST_ELEMENT_CAST (qtdemux),
            gst_message_new_element (GST_OBJECT_CAST (qtdemux), s));
    ...
    if (stream->new_stream || qtdemux->new_collection) {
      gchar *stream_id;
      GstEvent *event;
      ...
      if (qtdemux->collection) {
        stream_id =
            g_strdup_printf ("%s/%03u",
            gst_stream_collection_get_upstream_id (qtdemux->collection),
            stream->track_id);
      } else {
        stream_id =
       ...
       * problem. To prevent regression, let's block this.
       * Please let me fix when stream-collection for dolby-vision dual track is verified */
      if (qtdemux->collection && !qtdemux->is_dolby_hdr) {
        GstStream *collection_stream;
        GstStreamType stream_type;

        /* TODO: in case of dolby vision, let parsebin do stream-collection
         * What happen if CENC case ?? */
        switch (stream->subtype) {
          ...
          default:
          {
            ...
            if (g_str_has_prefix (padname, "video"))
              ...
              stream_type = GST_STREAM_TYPE_UNKNOWN;
            g_free (padname);
          }
            break;
        }
        ...
        collection_stream =
            gst_stream_new (stream_id, stream->caps, stream_type, stream_flags);

        /* FIXME: please remove this protected condition if it is verified */
        if (!stream->protected)
          gst_event_set_stream (event, collection_stream);

        gst_stream_set_caps (collection_stream, stream->caps);
        if (stream->pending_tags)
          gst_stream_set_tags (collection_stream, stream->pending_tags);

        gst_stream_collection_add_stream (qtdemux->collection,
            collection_stream);
      }
    ...
    if (qtdemux->collection && stream->caps) {
      stream->caps = gst_caps_make_writable (stream->caps);
      gst_caps_set_simple (stream->caps, "upstream-id", G_TYPE_STRING,
          gst_stream_collection_get_upstream_id (qtdemux->collection), NULL);
    }

    if (qtdemux->highest_temporal_id != -1) {
      stream->caps = gst_caps_make_writable (stream->caps);
      gst_caps_set_simple (stream->caps, "highest-temporal-id", G_TYPE_INT,
          qtdemux->highest_temporal_id, NULL);
      qtdemux->highest_temporal_id = -1;
    }

    if (qtdemux->preselection_id != -1) {
      stream->caps = gst_caps_make_writable (stream->caps);
      gst_caps_set_simple (stream->caps, "preselection-id", G_TYPE_INT,
          qtdemux->preselection_id, NULL);
      qtdemux->preselection_id = -1;
    }
...
gst_qtdemux_stream_check_and_change_stsd_index (GstQTDemux * demux,
    QtDemuxStream * stream)
{
  if (stream->stsd_entries_length < 2 ||
  ...
  qtdemux_sample_description_copy_into_stream (demux,
      &stream->stsd_entries[stream->cur_stsd_entry_index], stream);
}
...
done3:
  {
    ...
    if (stream->stss_present == TRUE) {
      ...
      if (!n_sample_syncs || (n_sample_syncs == n_samples)) {
  ...
  if ((edts = qtdemux_tree_get_child_by_type (trak, FOURCC_edts))) {
    ...
    for (i = 0; i < n_segments; i++) {
      ...
      if (rate_int <= 1) {
        ...
        /* some specific files  have a problem with rate_int parsing if edts header exists.
         * so we set the segment rate to 1 */
        segment->rate = 1;
      }
...
static gboolean
qtdemux_sample_description_copy_into_stream (GstQTDemux * qtdemux,
    QtDemuxStreamStsdEntry * stsd_entry, QtDemuxStream * stream)
{
  g_return_val_if_fail (stream != NULL, FALSE);
  g_return_val_if_fail (stsd_entry != NULL, FALSE);

  /* free allocated memory on stsd_entry if exist */
  if (stream->caps) {
    gst_caps_unref (stream->caps);
    stream->caps = NULL;
  }
  ...
  if (stsd_entry->caps)
    stream->caps = gst_caps_copy (stsd_entry->caps);

  stream->fourcc = stsd_entry->fourcc;

  stream->sampled = stsd_entry->sampled;
  stream->padding = stsd_entry->padding;

  stream->width = stsd_entry->width;
  stream->height = stsd_entry->height;

  stream->display_width = stsd_entry->display_width;
  stream->display_height = stsd_entry->display_height;

  stream->bits_per_sample = stsd_entry->bits_per_sample;
  stream->color_table_id = stsd_entry->color_table_id;
  if (stsd_entry->rgb8_palette)
    stream->rgb8_palette = gst_memory_ref (stsd_entry->rgb8_palette);

  stream->rate = stsd_entry->rate;
  stream->n_channels = stsd_entry->n_channels;
  stream->samples_per_packet = stsd_entry->samples_per_packet;
  stream->samples_per_frame = stsd_entry->samples_per_frame;
  stream->bytes_per_packet = stsd_entry->bytes_per_packet;
  stream->bytes_per_sample = stsd_entry->bytes_per_sample;
  stream->bytes_per_frame = stsd_entry->bytes_per_frame;
  stream->compression = stsd_entry->compression;

  return TRUE;
}

static gboolean
qtdemux_stream_copy_into_sample_description (GstQTDemux * qtdemux,
    QtDemuxStream * stream, gint stsd_index)
{
  QtDemuxStreamStsdEntry *stsd_entry;

  g_return_val_if_fail (stream != NULL, FALSE);
  g_return_val_if_fail (stsd_index >= 0, FALSE);
  g_return_val_if_fail (stsd_index < stream->stsd_entries_length, FALSE);

  stsd_entry = &stream->stsd_entries[stsd_index];
  ...
  if (stream->caps)
    stsd_entry->caps = gst_caps_copy (stream->caps);

  stsd_entry->fourcc = stream->fourcc;

  stsd_entry->sampled = stream->sampled;
  stsd_entry->padding = stream->padding;

  stsd_entry->width = stream->width;
  stsd_entry->height = stream->height;

  stsd_entry->display_width = stream->display_width;
  stsd_entry->display_height = stream->display_height;

  stsd_entry->bits_per_sample = stream->bits_per_sample;
  stsd_entry->color_table_id = stream->color_table_id;
  if (stream->rgb8_palette)
    stsd_entry->rgb8_palette = gst_memory_ref (stream->rgb8_palette);

  stsd_entry->rate = stream->rate;
  stsd_entry->n_channels = stream->n_channels;
  stsd_entry->samples_per_packet = stream->samples_per_packet;
  stsd_entry->samples_per_frame = stream->samples_per_frame;
  stsd_entry->bytes_per_packet = stream->bytes_per_packet;
  stsd_entry->bytes_per_sample = stream->bytes_per_sample;
  stsd_entry->bytes_per_frame = stream->bytes_per_frame;
  stsd_entry->compression = stream->compression;
...
qtdemux_parse_trak (GstQTDemux * qtdemux, GNode * trak)
{
  ...
  if (!qtdemux->got_moov) {
    ...
    if (!stream) {
       ...
       * This assumes that DASH segment has only one track. */
      /* FIXME: We may be able to handle this using DSC in the future */
      if (qtdemux->upstream_format_is_time && qtdemux->n_streams == 1 &&
          qtdemux->streams[0]) {
        stream = qtdemux->streams[0];
        ...
        if (QT_FOURCC ((guint8 *) hdlr->data + 16) == stream->subtype) {
          GST_DEBUG_OBJECT (qtdemux, "Update track_id from stream %u to %"
              G_GUINT32_FORMAT, stream->track_id, track_id);
    ...
    if (qtdemux->upstream_format_is_time && qtdemux->fragmented) {
      /* unset parsed_trex flag in order to apply new trex box's data */
      stream->parsed_trex = FALSE;
    }
...
/* FIXME: it may be better to use loop, but it may cause conflict with upstream
 * code due to indent difference, so let's use goto syntax until this approach
 * will be accepted in upstream */
stsd_entry_loop:
  GST_LOG_OBJECT (qtdemux, "stsd entry loop %d", stsd_index);
  ...
  if (fourcc == FOURCC_encv || fourcc == FOURCC_enca) {
    ...
    /* FIXME: This must be work around in my opinion...
     * Upstream should notify stream change with new stream-start event */
    if (qtdemux->upstream_format_is_time && !new_stream && !stream->protected) {
      GST_DEBUG_OBJECT (qtdemux, "stream changed from clear to encrypted");
      qtdemux->new_collection = TRUE;
    }
    stream->protected = TRUE;
  ...
  } else if (qtdemux->upstream_format_is_time && !new_stream
      && stream->protected) {
    /* FIXME: This must be work around in my opinion...
     * Upstream should notify stream change with new stream-start event */

    GST_DEBUG_OBJECT (qtdemux, "stream changed from encrypted to clear");
    qtdemux->new_collection = TRUE;
    stream->protected = FALSE;
  }
  ...
  if (stream->subtype == FOURCC_vide) {
    ...
    if (ceil (((double) stream->width / (double) 16)) *
        ceil ((double) stream->height / (double) 16) >= 8704) {
      qtdemux->isBigData = TRUE;
      GST_ERROR_OBJECT (qtdemux, "ceil(width/16) %4f, ceil(height/16) %4f",
          ceil ((double) (stream->width / 16)),
          ceil ((double) (stream->height / 16)));
    }
    ...
    if (!stream->protected) {
      if (mp4v == NULL || QTDEMUX_TREE_NODE_FOURCC (mp4v) != fourcc) {
        mp4v = NULL;
      }
    } else {
      if (mp4v == NULL || QTDEMUX_TREE_NODE_FOURCC (mp4v) != FOURCC_encv) {
        mp4v = NULL;
      }
    }
    ...
    if (esds) {
      ...
      switch (fourcc) {
...
#endif
        {
           ...
           */
          if (!qtdemux->dolby_vision_support && qtdemux->has_dolby_bl_cand
              && (fourcc == FOURCC_dvhe || fourcc == FOURCC_dvh1)
              && !qtdemux->fragmented) {
            GST_DEBUG_OBJECT (qtdemux, "ignore non SDR dolby-vision track");
            qtdemux->has_dolby_el_cand = FALSE;
            goto skip_track;
        ...
        case FOURCC_rak3:
        {
          /* Decoder does not support video codec of SVQ1 */
          GST_ELEMENT_ERROR (qtdemux, STREAM, DEMUX,
              (_("This file contains no playable stream.")),
              ("Because of unsupported codec"));
          gst_qtdemux_stream_free (qtdemux, stream);
          return FALSE;
        }
        case FOURCC_rle_:
    ...
    switch (fourcc) {
      case FOURCC_in24:
      {
        ...
        in24 = qtdemux_tree_get_child_by_index (stsd, stsd_index);
        if (!in24)
          break;
      ...
      case FOURCC_owma:
      {
        ...
        owma = qtdemux_tree_get_child_by_index (stsd, stsd_index);
        if (!owma)
          break;
        owma_data = owma->data;
      ...
      case FOURCC_opus:
      {
        ...
        opus = qtdemux_tree_get_child_by_index (stsd, stsd_index);
        if (!opus)
          break;
        opus_data = opus->data;
    ...
    if (stream->protected && fourcc == FOURCC_mp4a) {
      if (mp4a == NULL || QTDEMUX_TREE_NODE_FOURCC (mp4a) != FOURCC_enca) {
        mp4a = NULL;
      }
    } else {
      if (mp4a == NULL || QTDEMUX_TREE_NODE_FOURCC (mp4a) != FOURCC_mp4a) {
        mp4a = NULL;
      }
    }
    ...
    if ((fourcc & 0xffff) == (('s' << 8) | 'm')) {
      ...
      switch (fourcc) {
        ...
        case FOURCC_alac:
        {
          ...
          alac = qtdemux_tree_get_child_by_index (stsd, stsd_index);
          if (alac) {
    ...
    switch (fourcc) {
      case FOURCC_mp4s:
      {
        ...
        mp4s = qtdemux_tree_get_child_by_index (stsd, stsd_index);
        if (mp4s)
    ...
    GST_INFO_OBJECT (qtdemux,
        "type %" GST_FOURCC_FORMAT " caps %" GST_PTR_FORMAT,
        GST_FOURCC_ARGS (fourcc), stream->caps);
  } else if (stream->subtype == FOURCC_hint) {
    switch (fourcc) {
      case FOURCC_mmth:
      {
        GNode *mmth = NULL;
        const guint8 *mmth_data;
        guint32 len;

        mmth = qtdemux_tree_get_child_by_index (stsd, stsd_index);
        if (mmth) {
          guint16 hinttrackversion;     /* should be 1 */
          guint16 highestcompatibleversion;     /* should be 1 */
          guint16 packet_id;
          guint8 tmp;
          gboolean has_mfus_flag;
          gboolean is_timed;

          GST_DEBUG_OBJECT (qtdemux, "Found mmth");

          mmth_data = mmth->data;

          len = QT_UINT32 (mmth_data);

          if (len != 23) {
            GST_DEBUG_OBJECT (qtdemux, "mmth box length is too small");
            break;
          }

          hinttrackversion = QT_UINT16 (mmth_data + 16);
          highestcompatibleversion = QT_UINT16 (mmth_data + 16 + 2);
          packet_id = QT_UINT16 (mmth_data + 16 + 4);
          tmp = QT_UINT8 (mmth_data + 16 + 6);
          has_mfus_flag = (tmp >> 7) & 0x1;
          is_timed = (tmp >> 6) & 0x1;

          GST_DEBUG_OBJECT (qtdemux, "hint track version = %" G_GUINT16_FORMAT,
              hinttrackversion);
          GST_DEBUG_OBJECT (qtdemux, "highest compatible version = %"
              G_GUINT16_FORMAT, highestcompatibleversion);
          GST_DEBUG_OBJECT (qtdemux, "packet_id = %" G_GUINT16_FORMAT,
              packet_id);
          GST_DEBUG_OBJECT (qtdemux, "has_mfus_flag = %d, is_timed = %d",
              has_mfus_flag, is_timed);

          qtdemux->has_mmth = TRUE;
          qtdemux->is_mmth_timed = is_timed;
        }
        break;
      }
      default:
  ...
  qtdemux_stream_copy_into_sample_description (qtdemux, stream, stsd_index);
  stsd_index++;
  if (stsd_index < stsd_entry_count)
    goto stsd_entry_loop;

  /* reset stream's sample description data to the first stsd_entry data */
  if (stsd_entry_count > 1) {
    qtdemux_sample_description_copy_into_stream (qtdemux,
        &stream->stsd_entries[0], stream);
  }
  stream->cur_stsd_entry_index = 0;
...
qtdemux_expose_streams (GstQTDemux * qtdemux)
{
  ...
  if (qtdemux->collection && !qtdemux->is_dolby_hdr) {
    /* Emit collection message */
    gst_element_post_message (GST_ELEMENT_CAST (qtdemux),
        gst_message_new_stream_collection (GST_OBJECT_CAST (qtdemux),
            qtdemux->collection));

    /* Send collection event */
    gst_qtdemux_push_event (qtdemux,
        gst_event_new_stream_collection (qtdemux->collection));
  }
  ...
  for (iter = oldpads; iter; iter = g_slist_next (iter)) {
    ...
    /* FIXME: if refcount of this pad were controlled by demux, followng unref
     * is correct!! But, refcount does not matched what we expected. maybe
     * parsebin is doing it. Until fix it, temporarily disable this, because
     * this causes segfault in desktop environment */
...
static gboolean
qtdemux_is_keyframe_trick_condition (GstQTDemux * qtdemux)
{
  gboolean condition = FALSE;
  gdouble keyframe_trickrate = 4.0;
  if (qtdemux->isBigData)
    keyframe_trickrate = 2.0;

  if (qtdemux->segment.rate < 0.0
      || qtdemux->segment.rate >= keyframe_trickrate)
    condition = TRUE;
  return condition;
}
...
#if defined (MPEGDASH_MODE) || defined (ATSC3_MODE)
    if (QTDEMUX_IS_CUSTOM_MODE (qtdemux)) {
...
static void
qtdemux_set_custom_video_caps (GstQTDemux * qtdemux, QtDemuxStream * stream,
    GstCaps * caps)
{
  char *s, fourstr[5];
  ...
  g_return_if_fail (stream != NULL);
  g_return_if_fail (stream->fourcc);
    ...
    GNode * esds, GstTagList * list)
{
  ...
  if (caps) {
    ...
    if (stream->subtype == FOURCC_vide)
      qtdemux_set_custom_video_caps (qtdemux, stream, stream->caps);
  }
    ...
    guint32 fourcc, const guint8 * stsd_entry_data, gchar ** codec_name)
{
  ...
  switch (fourcc) {
    ...
    case GST_MAKE_FOURCC ('m', 'p', 'g', '1'):
      ...
      caps = gst_caps_new_simple ("video/mpeg", "mpegversion", G_TYPE_INT, 1,
          "systemstream", G_TYPE_BOOLEAN, FALSE, NULL);
      break;
    case GST_MAKE_FOURCC ('m', '2', 'v', '1'): /* MPEG2 */
    ...
    case GST_MAKE_FOURCC ('D', 'I', 'V', '3'):
      ...
      /* translate the fourcc divx to xvid */
  ...
  if (caps) {
    qtdemux_set_custom_video_caps (qtdemux, stream, caps);
  }
    ...
    guint32 fourcc, const guint8 * data, int len, gchar ** codec_name)
{
  ...
  switch (fourcc) {
    ...
    case FOURCC_raw_:
    {
      ...
      if (depth == 8) {
        ...
        /* add big endian for LPCM of audio */
        endian = G_BIG_ENDIAN;
    ...
    case 0x6d730002:
      ...
      caps = gst_caps_new_simple ("audio/x-adpcm", "layout", G_TYPE_STRING,
          "microsoft", "block_align", G_TYPE_INT, stream->bytes_per_frame,
          NULL);
      break;
    case 0x1100736d:
    case 0x6d730011:
      ...
      caps = gst_caps_new_simple ("audio/x-adpcm", "layout", G_TYPE_STRING,
          "dvi", "block_align", G_TYPE_INT, stream->bytes_per_frame, NULL);
      break;
    case 0x1700736d:
    case 0x6d730017:
      ...
      caps = gst_caps_new_simple ("audio/x-adpcm", "layout", G_TYPE_STRING,
          "quicktime" "block_align", G_TYPE_INT, stream->bytes_per_frame, NULL);
      break;
    case GST_MAKE_FOURCC ('.', 'm', 'p', '2'):
    case 0x2e6d7032:
      GST_DEBUG_OBJECT (qtdemux, "MPEG-1 layer 2");
    ...
    case FOURCC_ima4:
      ...
      caps = gst_caps_new_simple ("audio/x-adpcm", "layout", G_TYPE_STRING,
          "quicktime", "block_align", G_TYPE_INT, stream->bytes_per_frame,
          NULL);
      break;
    case FOURCC_alac:
  ...
  if (g_str_has_prefix (name, "audio/x-raw")) {
    stream->need_clip = TRUE;
    stream->max_buffer_size = stream->rate * stream->bytes_per_frame;
...
{
  if (scheme_type == FOURCC_cenc || scheme_type == FOURCC_cens ||
...
qtdemux_prepare_dolby_track (GstQTDemux * qtdemux, QtDemuxStream * stream)
{
  ...
  if (!qtdemux->dolby_vision_support) {
    ...
    gst_caps_set_simple (stream->caps, "dolby-vision", G_TYPE_BOOLEAN,
        TRUE, NULL);
    if (qtdemux->dv_profile >= 0 && qtdemux->dv_profile <= 7)
      gst_caps_set_simple (stream->caps, "dolby-vision-profile", G_TYPE_INT,
          qtdemux->dv_profile, NULL);
  }

  /* 2. Check single- or dual-track */
  if (qtdemux->has_dolby_bl_cand && qtdemux->has_dolby_el_cand) {
    gst_caps_set_simple (stream->caps, "need-compositor",
